...
class PaymentsPage extends StatefulWidget {
  ...
  const PaymentsPage({Key? key, this.avatarImagePath = 'assets/avatar.jpg'})
    : super(key: key);
...
class _PaymentsPageState extends State<PaymentsPage> {
  ...
  List<Map<String, dynamic>> get filteredPayments => payments.where((pmt) {
    final q = searchQuery.toLowerCase();
    return pmt['payment_id'].toLowerCase().contains(q) ||
        pmt['patient'].toLowerCase().contains(q) ||
        pmt['status'].toLowerCase().contains(q);
  }).toList();
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text(
              'Manage payment records below.',
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500),
            ),
            const SizedBox(height: 16),
            Row(
              children: [
                _infoCard('Total', '$total', Icons.payment, Colors.teal),
                _infoCard('Paid', '$paid', Icons.check_circle, Colors.green),
                _infoCard(
                  'Pending',
                  '$pending',
                  Icons.hourglass_top,
                ...
                _infoCard('Failed', '$failed', Icons.error, Colors.red),
              ],
            ),
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'Search payment ID, patient or status...',
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: DataTable(
                  ...
                  rows: filteredPayments.map((pmt) {
                    return DataRow(
                      cells: [
                        DataCell(Text(pmt['payment_id'])),
                        DataCell(Text(pmt['patient'])),
                        DataCell(Text(pmt['date'])),
                        DataCell(Text('\$${pmt['amount'].toStringAsFixed(2)}')),
                        DataCell(
                          Text(
                            pmt['status'],
                            style: TextStyle(
                              color: _statusColor(pmt['status']),
                            ),
                          ),
                        ),
                        DataCell(
